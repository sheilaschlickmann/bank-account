    ...
    try {
        ...
        for (const record of event.Records) {
            ...
            if (data.Count > 0) {
                ...
                valorTeste = tipoTeste == "DEBIT" ? (Number(data.Items[0].value) - Number(valorTeste)) : (Number(data.Items[0].value) + Number(valorTeste))
                idTeste = data.Items[0].id;
            }else {
                console.log('entrou no else');
                valorTeste = Number(valorTeste);
            }

            if (tipoTeste == "DEBIT") {
                console.log('entrou debito');
